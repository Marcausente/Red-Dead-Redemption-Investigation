-- UPGRADE DOJ INTERROGATIONS SYSTEM
...
ADD COLUMN IF NOT EXISTS meddoj_url TEXT;

-- 2. Drop old RPCs to avoid conflicts
DROP FUNCTION IF EXISTS get_doj_interrogations();
DROP FUNCTION IF EXISTS create_doj_interrogation(TEXT, TEXT, TEXT, UUID);

-- 3. Create New Manage RPC (Create, Update, Delete)
CREATE OR REPLACE FUNCTION manage_doj_interrogation(
...
BEGIN
  ...
  IF p_action = 'create' THEN
    INSERT INTO public.doj_interrogations (
        created_by, 
        title, 
        interrogation_date, 
        agents_present, 
  ...
  ELSIF p_action = 'update' THEN
    UPDATE public.doj_interrogations
    SET 
        title = p_title, 
  ...
  ELSIF p_action = 'delete' THEN
    DELETE FROM public.doj_interrogations WHERE id = p_id;

  ELSIF p_action = 'link' THEN
    UPDATE public.doj_interrogations SET case_id = p_case_id WHERE id = p_id;

  ELSIF p_action = 'unlink' THEN
    UPDATE public.doj_interrogations SET case_id = NULL WHERE id = p_id;
...
CREATE OR REPLACE FUNCTION get_doj_interrogations(p_case_id UUID DEFAULT NULL)
RETURNS TABLE (
  id UUID,
  title TEXT,
...
CREATE OR REPLACE FUNCTION get_available_doj_interrogations_to_link()
RETURNS TABLE (
  id UUID,
  title TEXT,
  created_at TIMESTAMP WITH TIME ZONE,
