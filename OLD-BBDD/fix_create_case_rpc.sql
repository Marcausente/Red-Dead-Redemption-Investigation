-- CRITICAL FIX: Drop the old function to prevent parameter mismatch
-- The old signature had 5 arguments, the new one has 6.
DROP FUNCTION IF EXISTS public.create_new_case(text, text, timestamp with time zone, text, uuid[]);

-- Also drop the new one just in case to ensure clean recreation
...
-- 2. Re-Create Create Case RPC (6 Params)
...
-- 3. Ensure get_case_details is also up to date
