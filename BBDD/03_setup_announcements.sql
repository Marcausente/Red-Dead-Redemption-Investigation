...
BEGIN
  ...
  SELECT
    ...
    (a.author_id = v_viewer_id OR v_viewer_role IN ('Administrador', 'Coordinador')) as cur_user_can_delete
...
BEGIN
  ...
  IF v_user_role NOT IN ('Agente BOI', 'Ayudante BOI', 'Coordinador', 'Administrador') THEN
    RAISE EXCEPTION 'Acceso denegado: No tienes permiso para colgar carteles en el tablón.';
  END IF;
...
BEGIN
  ...
  IF v_author_id IS NULL THEN
     RAISE EXCEPTION 'Cartel no encontrado';
  END IF;

  -- Logic: Allow edit if Author OR High Command
  IF auth.uid() = v_author_id OR v_user_role IN ('Administrador', 'Coordinador') THEN
    ...
    IF v_user_role IN ('Administrador', 'Coordinador') THEN
        UPDATE public.announcements SET pinned = p_pinned WHERE id = p_id;
    END IF;
  ELSE
    RAISE EXCEPTION 'Acceso denegado: No puedes editar este cartel.';
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  ELSE
    RAISE EXCEPTION 'Acceso denegado: No puedes arrancar este cartel.';
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  ELSE
    RAISE EXCEPTION 'Acceso denegado: Solo altos mandos pueden clavar carteles permanentemente.';
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
